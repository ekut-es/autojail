...
def check_project_config(c, autojailhouse_yml) -> bool:
    check_host = (
        lambda host: c.run(f"ping -W 1 -c 1 {host}", hide="both").exited == 0
    )
    valid = True
    ...
    if autojailhouse_yml["project_type"] == "automate":
        ...
        if not (
            cross_compiler_bin_path / (prefix + cross_compiler["cc"])
        ).exists():
            logging.error("Invalid CC compiler")
            valid = False

        if not (
            cross_compiler_bin_path / (prefix + cross_compiler["cxx"])
        ).exists():
...
def init(
    ...
    jailhouse_git=JAILHOUSE_REPO,
    cross_compiler=None,
    base_arch=None,
    ssh_login=None,
    cross_compiler_prefix=None,
    cross_compiler_cc="gcc",
    cross_compiler_cxx="g++",
    ...
    if (
        not kernel_build_dir
        or not jailhouse_git
        or not cross_compiler
        or not base_arch
    ):
        autojailhouse_yml = generate_config_automate(
            c, board_id, autojailhouse_yml
        )
    else:
    ...
    assert (
        check_project_config(c, autojailhouse_yml)
        and "Failed to create valid config"
    )
